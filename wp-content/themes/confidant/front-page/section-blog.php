<div class="front_page_section front_page_section_blog<?php
	$confidant_scheme = confidant_get_theme_option( 'front_page_blog_scheme' );
	if ( ! empty( $confidant_scheme ) && ! confidant_is_inherit( $confidant_scheme ) ) {
		echo ' scheme_' . esc_attr( $confidant_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( confidant_get_theme_option( 'front_page_blog_paddings' ) );
	if ( confidant_get_theme_option( 'front_page_blog_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$confidant_css      = '';
		$confidant_bg_image = confidant_get_theme_option( 'front_page_blog_bg_image' );
...
<?php
	// Add anchor
	$confidant_anchor_icon = confidant_get_theme_option( 'front_page_blog_anchor_icon' );
	$confidant_anchor_text = confidant_get_theme_option( 'front_page_blog_anchor_text' );
...
?>
	...
	<?php
	if ( confidant_get_theme_option( 'front_page_blog_fullheight' ) ) {
	...
	?>
			...
			<?php
			$confidant_css      = '';
			$confidant_bg_mask  = confidant_get_theme_option( 'front_page_blog_bg_mask' );
			$confidant_bg_color_type = confidant_get_theme_option( 'front_page_blog_bg_color_type' );
			if ( 'custom' == $confidant_bg_color_type ) {
				$confidant_bg_color = confidant_get_theme_option( 'front_page_blog_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_blog_content_wrap content_wrap">
			<?php
			// Caption
			$confidant_caption = confidant_get_theme_option( 'front_page_blog_caption' );
			if ( ! empty( $confidant_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_blog_caption front_page_block_<?php echo ! empty( $confidant_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $confidant_caption, 'confidant_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$confidant_description = confidant_get_theme_option( 'front_page_blog_description' );
			if ( ! empty( $confidant_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_blog_description front_page_block_<?php echo ! empty( $confidant_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $confidant_description ), 'confidant_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_blog_output">
				...
				if ( is_active_sidebar( 'front_page_blog_widgets' ) ) {
					...
					if ( ! confidant_exists_trx_addons() ) {
						...
						confidant_customizer_need_widgets_message( 'front_page_blog_caption', 'ThemeREX Addons - Blogger' );
					}
				}
				?>
			</div>
