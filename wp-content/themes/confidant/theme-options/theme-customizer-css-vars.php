...
if ( ! function_exists( 'confidant_add_css_vars' ) ) {
	add_filter( 'confidant_filter_get_css', 'confidant_add_css_vars', 1, 2 );
	function confidant_add_css_vars( $css, $args ) {
						...
						if ( is_array( $font ) ) {
							foreach ( $font as $css_prop => $css_value ) {
								...
								if ( empty( $suffix ) || ! empty( $font["{$css_prop}{$suffix}"] ) ) {
									...
									if ( ( ! empty( $font["{$param}{$suffix}"] ) && ! confidant_is_inherit( $font["{$param}{$suffix}"] ) ) || strpos( $css_prop, 'color' ) === false ) {
										$tmp .= "--theme-font-{$tag}_{$css_name}: " . ( ! empty( $font["{$param}{$suffix}"] ) && ! confidant_is_inherit( $font["{$param}{$suffix}"] )
																		? ( in_array( $css_prop, array( 'font-size', 'letter-spacing', 'margin-top', 'margin-bottom', 'border-width', 'border-radius' ) )
																			? confidant_prepare_css_value( $font["{$param}{$suffix}"] )
																			: $font["{$param}{$suffix}"]
																			)
																		: 'inherit'
																	) . ";\n";
		...
		if ( isset( $css['vars'] ) && isset( $args['vars'] ) ) {
			...
			if ( is_array( $CONFIDANT_STORAGE['options'] ) && count( $CONFIDANT_STORAGE['options'] ) > 0 ) {
				$breakpoints = confidant_get_theme_breakpoints();
				foreach ( $breakpoints as $bp => $bpv ) {
				...
				foreach ( $CONFIDANT_STORAGE['options'] as $option => $value ) {
					if ( ! empty( $value['css'] ) ) {
						...
						foreach ( $breakpoints as $bp => $bpv ) {
							...
							if ( $css_value !== '' ) {
								$css_value = confidant_prepare_css_value( $css_value );
		...
		if ( isset( $css['vars'] ) && isset( $args['vars'] ) ) {
			...
			if ( is_array( $vars ) && count( $vars ) > 0 ) {
				...
				if ( isset( $vars['sidebar_gap'] ) ) {
					$vars['sidebar_gap_width'] = confidant_prepare_css_value( $vars['sidebar_gap'] );
				}
				// Remove calculated values from css variables
				$exclude = apply_filters( 'confidant_filter_exclude_theme_vars', array( 'sidebar_gap' ) );	//Old case: array( 'sidebar_width', 'sidebar_gap' )
