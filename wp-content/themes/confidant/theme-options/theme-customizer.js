/* global confidant_color_schemes, confidant_dependencies, Color */
...
( function( api ) {
	...
	for (var i in confidant_color_schemes) {
		cssTemplate['scheme_' + i] = wp.template( 'confidant-color-scheme-' + i );
	}
	// Add Template with theme fonts
	cssTemplate['theme_fonts'] = wp.template( 'confidant-fonts' );
	// Add Template with theme vars
	cssTemplate['theme_vars'] = wp.template( 'confidant-vars' );

	// Allow an alpha channel in the color picker
	if ( confidant_customizer_vars['colorpicker_allow_alpha'] ) {
	...
	api.bind( 
		'ready', function() {
			...
			jQuery( '#customize-header-actions #save' )
				.before( '<input type="button" class="button customize-action-reset" value="' + confidant_customizer_vars['msg_reset'] + '">' );
			jQuery( '#customize-header-actions .customize-action-reset' )
				.on( 'click', function(e) {
					if (confirm( confidant_customizer_vars['msg_reset_confirm'] )) {
			...
			jQuery( '#customize-header-actions .spinner' )
				.after( '<button class="button customize-action-refresh icon-spin3" title="' + confidant_customizer_vars['msg_reload_title'] + '">' + confidant_customizer_vars['msg_reload'] + '</button>' );
			...
			if (confidant_customizer_vars && confidant_customizer_vars['theme_name_suffix']) {
				jQuery( '#customize-info .site-title' ).append( confidant_customizer_vars['theme_name_suffix'] );
			}
			...
			jQuery( '#customize-theme-controls [id^="customize-control-load_fonts"]' )
				.on( 'change', confidant_customizer_update_load_fonts );
			...
			jQuery( '#customize-theme-controls .control-section > .accordion-section-title' )
				.on( 'click', function() {
					...
					if (id !== '') {
						...
						if (section.length > 0) {
							confidant_customizer_check_dependencies( section );
						}
					}
				} );

			// Decorate subsections in the panel 'Typography'
			if ( confidant_customizer_vars['decorate_fonts'] ) {

				jQuery('body').addClass( 'confidant_decorate_fonts_section' );

				// Prepare section for open on title is clicked
				var confidant_customizer_title_clicked = function( title ) {
					var id = title.parent().attr( 'aria-owns' );
					confidant_customizer_close_opened_sections( title.parents( '#sub-accordion-panel-fonts' ), id );
					var section = jQuery( '#' + id ),
					...
					.each ( function() {
						...
						$self
							.on( 'keydown', function(e) {
								if ( e.which == 13 ) {
									confidant_customizer_title_clicked( $self );
								}
							} )
							.on( 'click', function() {
								confidant_customizer_title_clicked( $self );
				...
				var confidant_customizer_close_opened_sections = function( panel, current_id ) {
					if ( panel.length ) {
				...
				jQuery( '#customize-theme-controls #sub-accordion-panel-fonts .customize-panel-back' )
					.on( 'keydown', function(e) {
						if ( e.which == 13 ) {
							confidant_customizer_close_opened_sections( jQuery(this).parents('#sub-accordion-panel-fonts') );
						}
					} )
					.on( 'click', function() {
						confidant_customizer_close_opened_sections( jQuery(this).parents('#sub-accordion-panel-fonts') );
	...
	api.bind( 'change', function(obj) {
		...
		confidant_customizer_check_dependencies( jQuery( '#customize-theme-controls #customize-control-' + obj.id ).parents( '.control-section' ) );
		confidant_customizer_refresh_preview( obj );
	} );
	...
	api.bind( 'refresh_schemes', function() {
		...
		for (i in cssTemplate) {
			...
			i = i.replace( 'scheme_', '' );
			if (typeof confidant_color_schemes[i] === 'undefined') {
				delete cssTemplate['scheme_' + i];
				jQuery( '#tmpl-confidant-color-scheme-' + i ).remove();
			} else if (tpl_idx == '') {
				tpl_idx     = i;
				tpl_content = jQuery( '#tmpl-confidant-color-scheme-' + i ).html().trim();
			}
		}
		// Add new templates
		var regex = new RegExp( "\.scheme_" + tpl_idx, "g" );
		for (i in confidant_color_schemes) {
			if (typeof cssTemplate['scheme_' + i] === 'undefined') {
				jQuery( '#tmpl-confidant-color-scheme-' + tpl_idx )
				.clone()
				.attr( 'id', 'tmpl-confidant-color-scheme-' + i )
				.html( tpl_content.replace( regex, '.scheme_' + i ) )
				.insertAfter( jQuery( '#tmpl-confidant-color-scheme-' + tpl_idx ) );
				cssTemplate['scheme_' + i] = wp.template( 'confidant-color-scheme-' + i );
			}
		}
	} );
	...
	for (var action in confidant_customizer_vars['actions']['expand']) {
		if (action == 'length') {
			continue;
		}
		if (confidant_customizer_vars['actions']['expand'][action]['type'] == 'panel') {
			api.panel(
				action, function( panel ) {
					panel.expanded.bind(
						function( isExpanded ) {
							if ( isExpanded ) {
								var data = confidant_customizer_vars['actions']['expand'][panel.id];
			...
			api.section(
				action, function( section ) {
					section.expanded.bind(
						function( isExpanded ) {
							if ( isExpanded ) {
								var data = confidant_customizer_vars['actions']['expand'][section.id];
								if (typeof data['url'] !== 'undefined' && data['url'] !== '') {
									api.previewer.previewUrl.set( data['url'] );
								}
								if (typeof data['callback'] !== 'undefined' && data['callback'] !== '' && typeof window[data['callback']] === 'function') {
	...
	function confidant_customizer_get_field_value(fld) {
	...
	function confidant_customizer_check_dependencies(cont) {
		if ( typeof confidant_dependencies == 'undefined' || CONFIDANT_STORAGE['check_dependencies_now'] ) return;
			...
			function() {
				...
				if (depend) {
					...
					for (i in depend) {
						...
						if (fld.length > 0) {
							val = confidant_customizer_get_field_value( fld );
							if ( val == 'inherit' ) {
								dep_cnt = 0;
								dep_all = 1;
								var tag;
	...
	function confidant_customizer_refresh_preview(obj) {
		...
		if (updateCSS) {
			...
			if (id == 'scheme_storage') {
				...
				// Any color in the scheme_storage is changed
			} else if (Object.values( confidant_sorted_schemes ).indexOf( id ) >= 0) {
				forceUpdateCSS = true;
				...
				if (var_name !== undefined) {
					...
					confidant_customizer_update_theme_vars( jQuery( '[data-customize-setting-link="' + id + '"]' ).data( 'var_name' ), val );
					processed = true;
				}

				// Any theme fonts parameter is changed
			} else {
				for (opt in confidant_theme_fonts) {
					for (rule in confidant_theme_fonts[opt]) {
						if (opt + '_' + rule.replace( ':', '-' ) == id) {
							// Store new value to the fonts table
							confidant_customizer_update_theme_fonts( opt, rule, val );
							processed = true;
							break;
						}
					}
			...
			if (processed || forceUpdateCSS) {
				confidant_customizer_update_css();
			}
		}
	...
	// Actions buttons
	function confidant_customizer_actions(e) {
	...
	function confidant_customizer_update_theme_vars(opt, value) {
		confidant_theme_vars[opt] = parseFloat( value );
	}

	// Store new value in the theme fonts
	function confidant_customizer_update_theme_fonts(opt, rule, value) {
		confidant_theme_fonts[opt][rule] = value;
	}

	// Change theme fonts options if load fonts is changed
	function confidant_customizer_update_load_fonts() {
		var opt_list = [], i, tag, sel, opt, name = '', family = '', val = '', new_val = '', sel_idx = 0;
		updateCSS    = false;
		for (i = 1; i <= confidant_customizer_vars['max_load_fonts']; i++) {
	...
	function confidant_customizer_update_css() {
		...
		var vars = confidant_customizer_add_theme_vars( confidant_theme_vars );

		// Make styles and add its to the css
		css += confidant_customizer_prepare_html_value( cssTemplate['theme_vars']( vars ) );

		// Add theme specific fonts rules
		var fonts = confidant_customizer_add_theme_fonts( confidant_theme_fonts );

		// Make styles and add its to the css
		css += confidant_customizer_prepare_html_value( cssTemplate['theme_fonts']( fonts ) );

		// Add colors
		var sorted = {}, scheme = '';
		for (var param in confidant_sorted_schemes) {
			if (param == 'length') {
				continue;
			}
			scheme = api( confidant_sorted_schemes[param] )();
			if (scheme && scheme !== 'inherit') {
				sorted[scheme] = 1;
			}
		}
		for (scheme in confidant_color_schemes) {
			sorted[scheme] = 1;
		}
		for (scheme in sorted) {
			var i, colors = [];
			// Copy all colors to the temp var
			for (i in confidant_color_schemes[scheme].colors) {
				colors[i] = confidant_color_schemes[scheme].colors[i];
			}
			// Add theme specific colors and tints
			colors = confidant_customizer_add_theme_colors( colors );
			...
			var tmpl = jQuery( '#tmpl-confidant-color-scheme-' + scheme ).html().trim();
			for (i in colors) {
	...
	function confidant_customizer_add_theme_colors(colors) {
		if (confidant_additional_colors) {
			var clr = '', v = '';
			for (var k in confidant_additional_colors) {
				v   = confidant_additional_colors[k];
				clr = colors[v['color']];
			...
			for (var css_prop in fonts[tag]) {
				...
				rez[tag + '_' + css_name] = css_value !== '' && css_value != 'inherit'
												? css_prop + ':' + ( ['font-size', 'letter-spacing', 'margin-top', 'margin-bottom', 'border-width', 'border-radius'].indexOf(css_prop) >= 0
																		? confidant_customizer_prepare_css_value( css_value )
																		: css_value
																		)
																	+ ';'
												: '';
	...
	function confidant_customizer_add_theme_vars(vars) {
		...
		if ( typeof vars['rad'] != 'undefined' ) {
			...
			rez['rad']      = confidant_customizer_prepare_css_value( vars['rad'] );
		...
		if ( typeof vars['page_width'] != 'undefined' ) {
			...
			if ( isNaN( vars['page_width'] ) || vars['page_width'] === 0) {
				vars['page_width'] = confidant_customizer_vars['page_width_default'];
			}
			vars['sidebar_width']       = parseInt( vars['sidebar_width'], 10 );
			vars['sidebar_gap']         = parseInt( vars['sidebar_gap'], 10 );

			rez['page_width']          = confidant_customizer_prepare_css_value( vars['page_width'] );
			rez['page_boxed_extra']     = confidant_customizer_prepare_css_value( vars['page_boxed_extra'] );
			rez['page_fullwide_extra']  = confidant_customizer_prepare_css_value( vars['page_fullwide_extra'] );
			rez['page_fullwide_max']    = confidant_customizer_prepare_css_value( vars['page_fullwide_max'] );
			rez['grid_gap']             = confidant_customizer_prepare_css_value( vars['grid_gap'] );
			rez['sidebar_prc']          = vars['sidebar_width'] / vars['page_width'];
			rez['sidebar_gap_prc']      = vars['sidebar_gap'] / vars['page_width'];
			rez['sidebar_width']        = confidant_customizer_prepare_css_value( vars['sidebar_width'] );
			rez['sidebar_gap']          = confidant_customizer_prepare_css_value( vars['sidebar_gap'] );
			rez['sidebar_gap_width']    = confidant_customizer_prepare_css_value( vars['sidebar_gap'] );
	...
	// Add ed to css value
	function confidant_customizer_prepare_css_value(val) {
		if (val !== '' && val != 'inherit') {
			var ed = ('' + val).slice( -1 );
			if ('0' <= ed && ed <= '9') {
				val += 'px';
	...
	function confidant_customizer_prepare_html_value(val) {
