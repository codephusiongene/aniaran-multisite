...
jQuery( document ).ready(
	function() {
		...
		confidant_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="confidant_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.confidant_post_editor' );
						if ( link.length > 0 ) {
							confidant_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.confidant_post_editor', function(e) {
						confidant_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('confidant_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function confidant_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
