...
?><div id="post-<?php the_ID(); ?>" <?php post_class( 'related_item post_format_' . esc_attr( $confidant_post_format ) ); ?> data-post-id="<?php the_ID(); ?>">
	...
	confidant_show_post_featured(
		array(
			'thumb_size'    => apply_filters( 'confidant_filter_related_thumb_size', confidant_get_thumb_size( (int) confidant_get_theme_option( 'related_posts' ) == 1 ? 'big' : 'med' ) ),
		)
	);
	?>
	<div class="post_header entry-header">
		...
		if ( in_array( get_post_type(), array( 'post', 'attachment' ) ) ) {
			?>
			<div class="post_meta">
				<a href="<?php echo esc_url( $confidant_link ); ?>" class="post_meta_item post_date"><?php echo wp_kses_data( confidant_get_date() ); ?></a>
			</div>
			<?php
		}
		?>
