...
if ( apply_filters( 'confidant_filter_single_post_header', is_singular( 'post' ) || is_singular( 'attachment' ) ) ) {
	...
	ob_end_clean();
	$confidant_with_featured_image = confidant_is_with_featured_image( $confidant_post_header /*, array( 'with_gallery' )*/);
	// Post title and meta
	ob_start();
	...
	if ( strpos( $confidant_post_header, 'post_featured' ) !== false
		...
		<div class="post_header_wrap post_header_wrap_in_header post_header_wrap_style_<?php
            ...
            if ( $confidant_with_featured_image ) {
                echo ' with_featured_image' . ( false && confidant_get_theme_option( 'single_parallax' ) == 0 ? ' confidant-full-height' : '' );
            }
		?>">
			<?php
