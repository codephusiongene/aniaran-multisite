...
if ( ! function_exists( 'confidant_storage_get' ) ) {
	/**
	 ...
	 */
	function confidant_storage_get( $var_name, $default = '' ) {
		global $CONFIDANT_STORAGE;
		return isset( $CONFIDANT_STORAGE[ $var_name ] ) ? $CONFIDANT_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'confidant_storage_set' ) ) {
	/**
	 ...
	 */
	function confidant_storage_set( $var_name, $value ) {
		global $CONFIDANT_STORAGE;
		$CONFIDANT_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'confidant_storage_empty' ) ) {
	/**
	 ...
	 */
	function confidant_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $CONFIDANT_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $CONFIDANT_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $CONFIDANT_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $CONFIDANT_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'confidant_storage_isset' ) ) {
	/**
	 ...
	 */
	function confidant_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $CONFIDANT_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $CONFIDANT_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $CONFIDANT_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $CONFIDANT_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'confidant_storage_unset' ) ) {
	/**
	 ...
	 */
	function confidant_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $CONFIDANT_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			if ( isset( $CONFIDANT_STORAGE[ $var_name ][ $key ][ $key2 ] ) ) {
				unset( $CONFIDANT_STORAGE[ $var_name ][ $key ][ $key2 ] );
			}
		} elseif ( '' !== $key ) {
			if ( isset( $CONFIDANT_STORAGE[ $var_name ][ $key ] ) ) {
				unset( $CONFIDANT_STORAGE[ $var_name ][ $key ] );
			}
		} else {
			if ( isset( $CONFIDANT_STORAGE[ $var_name ] ) ) {
				unset( $CONFIDANT_STORAGE[ $var_name ] );
			}
		}
	}
}

if ( ! function_exists( 'confidant_storage_inc' ) ) {
	/**
	 ...
	 */
	function confidant_storage_inc( $var_name, $value = 1 ) {
		global $CONFIDANT_STORAGE;
		if ( empty( $CONFIDANT_STORAGE[ $var_name ] ) ) {
			$CONFIDANT_STORAGE[ $var_name ] = 0;
		}
		$CONFIDANT_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'confidant_storage_concat' ) ) {
	/**
	 ...
	 */
	function confidant_storage_concat( $var_name, $value ) {
		global $CONFIDANT_STORAGE;
		if ( empty( $CONFIDANT_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'confidant_storage_get_array' ) ) {
	/**
	 ...
	 */
	function confidant_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $CONFIDANT_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $CONFIDANT_STORAGE[ $var_name ][ $key ] ) ? $CONFIDANT_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $CONFIDANT_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $CONFIDANT_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'confidant_storage_set_array' ) ) {
	/**
	 ...
	 */
	function confidant_storage_set_array( $var_name, $key, $value ) {
		global $CONFIDANT_STORAGE;
		if ( ! isset( $CONFIDANT_STORAGE[ $var_name ] ) ) {
			$CONFIDANT_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $CONFIDANT_STORAGE[ $var_name ] ) ) {
			if ( '' === $key ) {
				$CONFIDANT_STORAGE[ $var_name ][] = $value;
			} else {
...
if ( ! function_exists( 'confidant_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function confidant_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $CONFIDANT_STORAGE;
		if ( ! isset( $CONFIDANT_STORAGE[ $var_name ] ) ) {
			$CONFIDANT_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $CONFIDANT_STORAGE[ $var_name ][ $key ] ) ) {
			$CONFIDANT_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( is_array( $CONFIDANT_STORAGE[ $var_name ][ $key ] ) ) {
			if ( '' === $key2 ) {
				$CONFIDANT_STORAGE[ $var_name ][ $key ][] = $value;
			} else {
				$CONFIDANT_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
			}
		}
	}
}

if ( ! function_exists( 'confidant_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function confidant_storage_merge_array( $var_name, $key, $value ) {
		global $CONFIDANT_STORAGE;
		if ( ! isset( $CONFIDANT_STORAGE[ $var_name ] ) ) {
			$CONFIDANT_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$CONFIDANT_STORAGE[ $var_name ] = array_merge( $CONFIDANT_STORAGE[ $var_name ], $value );
		} else {
			$CONFIDANT_STORAGE[ $var_name ][ $key ] = array_merge( $CONFIDANT_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'confidant_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function confidant_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $CONFIDANT_STORAGE;
		if ( ! isset( $CONFIDANT_STORAGE[ $var_name ] ) ) {
			$CONFIDANT_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			confidant_array_insert_after( $CONFIDANT_STORAGE[ $var_name ], $after, $key );
		} else {
			confidant_array_insert_after( $CONFIDANT_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'confidant_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function confidant_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $CONFIDANT_STORAGE;
		if ( ! isset( $CONFIDANT_STORAGE[ $var_name ] ) ) {
			$CONFIDANT_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			confidant_array_insert_before( $CONFIDANT_STORAGE[ $var_name ], $before, $key );
		} else {
			confidant_array_insert_before( $CONFIDANT_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'confidant_storage_push_array' ) ) {
	/**
	 ...
	 */
	function confidant_storage_push_array( $var_name, $key, $value ) {
		global $CONFIDANT_STORAGE;
		if ( ! isset( $CONFIDANT_STORAGE[ $var_name ] ) ) {
			$CONFIDANT_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $CONFIDANT_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $CONFIDANT_STORAGE[ $var_name ][ $key ] ) ) {
				$CONFIDANT_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $CONFIDANT_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'confidant_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function confidant_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $CONFIDANT_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $CONFIDANT_STORAGE[ $var_name ] ) && is_array( $CONFIDANT_STORAGE[ $var_name ] ) && count( $CONFIDANT_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $CONFIDANT_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $CONFIDANT_STORAGE[ $var_name ][ $key ] ) && is_array( $CONFIDANT_STORAGE[ $var_name ][ $key ] ) && count( $CONFIDANT_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $CONFIDANT_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'confidant_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function confidant_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $CONFIDANT_STORAGE;
		if ( ! isset( $CONFIDANT_STORAGE[ $var_name ] ) ) {
			$CONFIDANT_STORAGE[ $var_name ] = array();
		}
		if ( empty( $CONFIDANT_STORAGE[ $var_name ][ $key ] ) ) {
			$CONFIDANT_STORAGE[ $var_name ][ $key ] = 0;
		}
		$CONFIDANT_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'confidant_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function confidant_storage_concat_array( $var_name, $key, $value ) {
		global $CONFIDANT_STORAGE;
		if ( ! isset( $CONFIDANT_STORAGE[ $var_name ] ) ) {
			$CONFIDANT_STORAGE[ $var_name ] = array();
		}
		if ( empty( $CONFIDANT_STORAGE[ $var_name ][ $key ] ) ) {
			$CONFIDANT_STORAGE[ $var_name ][ $key ] = '';
		}
		$CONFIDANT_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'confidant_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function confidant_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $CONFIDANT_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $CONFIDANT_STORAGE[ $var_name ] ) ? $CONFIDANT_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $CONFIDANT_STORAGE[ $var_name ] ) ? $CONFIDANT_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'confidant_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function confidant_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $CONFIDANT_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $CONFIDANT_STORAGE[ $var_name ]->$prop ) ? $CONFIDANT_STORAGE[ $var_name ]->$prop : $default;
	}
}
